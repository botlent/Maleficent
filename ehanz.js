...
previewType: "PHOTO",
 thumbnailUrl: photo,
 sourceUrl: 'https://github.com/Alfisyahriall'
   }
  },
 text: teks
  };
return ehz.sendMessage(m.chat, ngel, { quoted: ftoko});
};
...
Hi kak ${pushname} 👋 ada yang bisa Maleficent bantu?
 `
  await sleep(1000)
ehz.sendMessage(m.chat,{contextInfo, text:teks},{quoted:ftoko})
                                                                              }
}   
...
ehz.sendMessage(from, {sticker: oti },{quoted: ftoko})
}
//=============
const jan = fuck
...
ehz.sendMessage(from, {sticker: kocak },{quoted: ftoko})
}
//==========
const pedo = paan
...
ehz.sendMessage(from, {sticker: prosi },{quoted: ftoko})
}
//==========
const weh = marah
...
ehz.sendMessage(from, {sticker: jaga },{quoted: ftoko})
}
//==========
const toxc = bad
...
ehz.sendMessage(from, {sticker: eeh },{quoted: ftoko})
}
//System Expired
 ...
 /* ~~~~~~~~~ RESPON USER AFK ~~~~~~~~~ */
        if (m.isGroup && !m.key.fromMe) {
            ...
            if (afk.checkAfkUser(m.sender, _afk)) {
                ...
                ehz.sendTextWithMentions(m.chat, `@${m.sender.split('@')[0]} telah kembali dari afk`, ftoko)
            }
        }
...
await ehz.sendMessage(from, { contextInfo: {externalAdReply: {showAdAttribution: true, title: 'Maleficent°᭄ᴮᵒᵗ', mediaType: 3,  renderLargerThumbnail : true,thumbnailUrl: vid.url,sourceUrl: `${sgc}`																												
...
.on("finish", async () => {  
await ehz.sendMessage(from, { audio:  fs.readFileSync(mp3File), mimetype: 'audio/mp4' },{ quoted: ftoko })
...
ehz.sendMessage(from, {text: yameteh},{quoted: ftoko})
}

const onlyGlimit = async() => {
let kyahh = `Maaf kak @${sender.split('@')[0]} limit game kamu sudah habis!`
ehz.sendMessage(from, {text: kyahh},{quoted: ftoko})
}
...
Hallo ${pushname} bot di setting hanya untuk di gunakan
di group , untuk mendapatkan fitur lengkap silahkan join group dibawah ini :
https://chat.whatsapp.com/H3XdID4WcwqE6AfxIDdRqc
...
└─────────────────`
ehz.sendMessage(from, {text: teks},{quoted: ftoko})
}
...
ehz.sendMessage(from,{text:`Limit kamu tersisa ${db.data.users[sender].limit}`}, {quoted: ftoko})
}
...
ehz.sendMessage(from, {text: `${caption}`, mentions: [room.terjawab + '@s.whatsapp.net']}, {quoted:ftoko}).then(msg => {
...
klik https://wa.me/${botNumber.split`@`[0]}`, ftoko, { mentions: [roof.p, roof.p2] })
	    ...
	    ehz.sendTextWithMentions(m.chat, `@${(roof.pilih ? roof.p2 : roof.p).split`@`[0]} tidak memilih suit, game berakhir`, ftoko)
	    }
	    delete ehz.suit[roof.id]
	    return !0
	    }, roof.timeout)
...
let { key } = await ehz.sendMessage(m.chat, {text: 'wait!!'}, {quoted: ftoko})//Pengalih isu
...
case  'getppgc':
	...
	await ehz.sendMessage(from, { image: { url: ppimg }}, { quoted: ftoko })
	break
...
ehz.sendMessage(from, { text: q ? q : '', mentions: mem }, {quoted: ftoko})
break
case "tagall": {
if (!isOwner && !isAdmins) return setReply(mess.only.admin)
if (!isGroup) return setReply(mess.only.group)
...
ehz.sendMessage(from, { text: teks, mentions: participants.map(a => a.id) }, { quoted: ftoko })
}
break
case 'kick': {
if (!isOwner && !isAdmins) return setReply(mess.only.admin)
...
ehz.sendMessage(from, { text: jawab, mentions: menst },{quoted: ftoko}) 
}
break
case 'jodohku':{
if (!isGroup) return onlyGroup()
...
case 'caridoi':
   ...
   if (!isGroup) return onlyGroup()
    ...
    setTimeout(() => {
    ehz.sendContAngel(from, nomerTemen, namTemen, ftoko)
    }, 9000)
    }
    break
            case 'afk':
...
ehz.sendTextWithMentions(from, teks, ftoko)
}
break
  case 'listpc': {
if(!isOwner) return setReply(mess.only.owner)
...
ehz.sendMessage(62895615063060 + "@s.whatsapp.net",{image: { url: `https://telegra.ph/file/9e61a5b20f29477ae8dee.jpg`}, caption: text12, mentionedJid:[sender], quoted: ftoko })
}
break
case 'accerror': {
...
ehz.sendMessage(from, { contextInfo: {externalAdReply: {showAdAttribution: true, title: `Maleficent°᭄ᴮᵒᵗ`, mediaType: 3,  renderLargerThumbnail : true,thumbnailUrl: 'https://telegra.ph/file/ec71f55d49a271c675926.jpg',sourceUrl: `https://wa.me/${nomerOwner}`
...
await ehz.sendImageAsSticker(m.sender, agg.result.sticker[ki], ftoko, { packname: "©Sticker", author: "Created By  Maleficent MD" })
} }
 break
 case 'sc': case 'script':{
...
Silahkan @${m.mentionedJid[0].split`@`[0]} untuk ketik terima/tolak`
            ehz.suit[id] = {
            chat: await ehz.sendText(m.chat, caption, ftoko, { mentions: parseMention(caption) }),
            id: id,
            p: sender,
            p2: m.mentionedJid[0],
            status: 'wait',
            waktu: setTimeout(() => {
            if (ehz.suit[id]) ehz.sendText(m.chat, `_Waktu suit habis_`, ftoko)
            delete ehz.suit[id]
...
if (!isGroup) return onlyGroup()
	...
	ehz.sendMessage(from, { caption: teks, image: gambar}, {quoted: ftoko})
...
for (let i = 0; i < total; i ++){
   ...
   //ehz.sendMessage(from, {text: top, mentions: arrTop}, {quoted: ftoko})
...
case 'tiktok':
 ...
 await ehz.sendMessage(m.chat, {video: { url: e.result.link }, caption: ee }, { quoted: ftoko })
 break
        case 'ttmp3': 
case 'tiktokaudio':
            case 'ttaudio': {
...
setReply(mess.wait)
                ...
                ehz.sendMessage(m.chat, { audio: { url: i.result }, mimetype: 'audio/mp4' }, { quoted: ftoko })
            }
            break
...
ehz.sendMessage(m.chat, { image: { url: result }, caption: 'â­” Media Url : '+result }, { quoted: m })
}
break
        case 'ttimg':{
...
sendSticker(stikot)
                ...
                ehz.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/upscale?apikey=${api.Lol}&img=${anu}` }, caption: "nih kak >\<" }, { quoted: ftoko })}
            break
...
`
ehz.sendMedia(from, res.play, ftoko, {caption: caption})
...
ehz.sendMessage(from,{image, caption},{quoted: ftoko})
} 
break
case 'nsfw': {
...
ehz.sendMessage(m.sender, { caption: `Random Anime ${command}`, image: { url: `https://botapi.takimtod.repl.co/api/nsfw/${command}?apikey=BetaBotz`}, quoted: ftoko })
  db.data.users[sender].limit -= 1  
}
break
...
.then(({data}) => {
ehz.sendImageAsSticker(m.chat, data.url, ftoko, { packname: global.packName, author: global.authorName })
})
break
case 'anime':{
if (!isGroup) return onlyGroup()
...
await ehz.sendFile(m.chat, cita, 'image.jpg', `_${command}_`, ftoko)
...
ehz.sendMessage(m.chat, { image: { url: hehe }, caption: `_${command}_`}, { quoted: ftoko})
}
break
 //========== TOOLS ===========//
 case 'tomp4': 
...
if (!isGroup) return onlyGroup()
                ...
                await ehz.sendMessage(m.chat, { video: { url: webpToMp4.result, caption: 'Convert Webp To Video' } }, { quoted: ftoko })
...
if (!isGroup) return onlyGroup()
                ...
                exec(`ffmpeg -i ${media} ${ran}`, (err) => {
                    ...
                    ehz.sendMessage(m.chat, { image: buffer }, { quoted: ftoko })
                    fs.unlinkSync(ran)
                })
...
if (!isGroup) return onlyGroup()
            ...
            ehz.sendMessage(m.chat, {audio: audio, mimetype: 'audio/mpeg'}, { quoted : ftoko })
...
if (!isGroup) return onlyGroup()
	...
	await ehz.sendMessage(from, {audio: jadie, mimetype: 'audio/mp4' }, {quoted: ftoko})
	fs.unlinkSync(rname)
...
if (!isGroup) return onlyGroup()
            ...
            ehz.sendMessage(m.chat, {audio: audio, mimetype:'audio/mpeg', ptt:true }, {quoted:ftoko})
...
if (!isGroup) return onlyGroup()
                ...
                let media = await ehz.downloadAndSaveMediaMessage(qmsg)
                let webpToMp4 = await webp2mp4File(media)
                await ehz.sendMessage(m.chat, { video: { url: webpToMp4.result, caption: 'Convert Webp To Video' }, gifPlayback: true }, { quoted: ftoko })
                await fs.unlinkSync(media)
...
if (!isGroup) return onlyGroup()
                ...
                ehz.sendMessage(m.chat, { audio: buff, mimetype: 'audio/mpeg' }, { quoted : ftoko })
                fs.unlinkSync(ran)
                })
...
case "fetch":
  if (!q) {
    return m.reply(`Contoh:\n${prefix + command} https://github.com/Alfisyahriall`);
  }
  if (!/^https?:\/\//.test(q)) {
  ...
  if (body.match(/(mp4)/gi)) {
    fetch(`${q}`, requestOptions)
      .then((res) => ehz.sendMessage(from, { video: { url: `${q}` }, mimetype: "video/mp4", caption: "Success" }, { quoted: ftoko }))
      .catch((error) => m.reply("Error", error));
  } else if (body.match(/(mp3)/gi)) {
    fetch(`${q}`, requestOptions)
      .then((res) => ehz.sendMessage(from, { audio: { url: `${q}` }, mimetype: "audio/mp4", fileName: "Audio" }, { quoted: ftoko }))
      .catch((error) => m.reply("Error", error));
  } else if (body.match(/(png|jpg|jpeg)/gi)) {
    fetch(`${q}`, requestOptions)
      .then((res) => ehz.sendMessage(from, { image: { url: `${q}` }, caption: "Success" }, { quoted: ftoko }))
      .catch((error) => m.reply("Error", error));
  } else {
    fetch(`${q}`, requestOptions)
...
ehz.sendMessage(from, { contextInfo: {externalAdReply: {showAdAttribution: true, title: `Maleficent°᭄ᴮᵒᵗ`, mediaType: 3,  renderLargerThumbnail : true,thumbnailUrl:"https://telegra.ph/file/07e2ff838762aab24008e.jpg",sourceUrl: `https://wa.me/${nomerOwner}`
...
ehz.sendMessage(from, { contextInfo: {externalAdReply: {showAdAttribution: true, title: `Maleficent°᭄ᴮᵒᵗ`, mediaType: 3,  renderLargerThumbnail : true,thumbnailUrl: 'https://telegra.ph/file/048acba09543c168246e5.jpg',sourceUrl: `https://wa.me/${nomerOwner}`
}}, image: {url:buffer}, caption: `${mess.success}`}, { quoted: ftoko})
.catch((err) => setReply('Server sedang error coba lagi besok!'))
db.data.users[sender].limit -= 1
}
...
ehz.sendMessage(from, { contextInfo: {externalAdReply: {showAdAttribution: true, title: `${botName}`, mediaType: 3,  renderLargerThumbnail : true,thumbnailUrl: 'https://telegra.ph/file/048acba09543c168246e5.jpg',sourceUrl: `https://wa.me/${nomerOwner}`
}}, image: {url:buffer}, caption: `${mess.success}`}, { quoted: ftoko})
.catch((err) => setReply('Server sedang error coba lagi besok!'))
db.data.users[sender].limit -= 1
}
...
let tod = body.split("\n");
let pjr = tod[Math.floor(Math.random() * tod.length)];
ehz.sendImageAsSticker(from, pjr, ftoko)
}).catch((err) => setReply('Server sedang error coba lagi besok!'))
db.data.users[sender].limit -= 1
}
break
...
case 'smeme': {
	        ...
	        let FaTiH = await ehz.sendImageAsSticker(m.chat, smeme, ftoko, { packname: global.packName, author: global.authorName })
...
case 'take': case 'swm': {
            ...
            setReply(mess.wait)
                if (/image/.test(mime)) {
                    let media = await ehz.downloadMediaMessage(qmsg)
                    let encmedia = await ehz.sendImageAsSticker(m.chat, media, ftoko, { pacname: teks1, author: teks2 })
                    await fs.unlinkSync(encmedia)
                } else if (/video/.test(mime)) {
                    if ((quoted.msg || quoted).seconds > 11) return setReply('Maksimal 10 detik!')
                    let media = await ehz.downloadMediaMessage(qmsg)
                    let encmedia = await ehz.sendVideoAsSticker(m.chat, media, ftoko, { packname: teks1, author: teks2 })
                    await fs.unlinkSync(encmedia)
                } else {
 ...
 case 'attp':
                ...
                setReply(mess.wait)
                ehz.sendMessage(m.chat, { sticker: { url: `https://api.lolhuman.xyz/api/attp?apikey=${api.Lol}&text=${q}`} }, { quoted: ftoko })
                break
case 'ttp':{
	...
	let nono = await jancok.toFile(stok)
	let nah = fs.readFileSync(nono)
	await ehz.sendMessage(from,{sticker: nah},{quoted: ftoko})
	await fs.unlinkSync(stok)
...
case 'tts':
	...
	if (ras) ehz.sendMedia(from, ras, ftoko, {caption: "Nih"})
...
ehz.sendMessage(from,{caption: 'Dosa Tanggung  Sendiri Dah',video:{url: nano.video_1}},{quoted:ftoko})
...
ehz.sendMessage(m.chat,{video:{url:hayu},caption:`nih kak🗿`},{quoted: ftoko})
}
break 
...
ehz.sendMessage(from, { text : `Hai Kak @${sender.split("@")[0]}, Hii itu nomor owner ku`, contextInfo:{
forwardingScore: 9999999, 
isForwarded: true,
mentionedJid:[sender]
}}, { quoted: ftoko })
...
case 'bucinsertifikat':
			...
			case 'bucinserti':
				if (args.length == 0) return setReply(`Example: ${prefix + command} Alfi & angel\n bjir lah love you`)
				setReply(mess.wait)
				let kueri = args.join(" ")
                ehz.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/bucinserti?apikey=${api.Lol}&name=${kueri}`}, caption: 'Sertifikatnya kack'}, {quoted: ftoko})
            break
case 'tololserti':
			case 'tololcert':
			...
			setReply(mess.wait)
			let ytta = args.join(" ")
            ehz.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/toloserti?apikey=${api.Lol}&name=${ytta}`}, caption: 'Sertifikatnya kack'}, {quoted: ftoko})
            break
case 'pacarsertifikat':
            ...
            ehz.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/pacarserti?apikey=${api.Lol}&name1=${nik}&name2=${prov}`}, caption: titidnya}, {quoted: ftoko})
            break
case 'pornhub':
		case 'glitch':
		case 'avenger':
		...
		case 'wallgravity':
			...
			setReply(mess.wait)
			ehz.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/textprome2/${command}?apikey=${api.Lol}&text1=${args[0]}&text2=${args[1]}`}, caption: `© Maleficent MDz\n\n Type: ${command}`})
			break
case 'blackpink':
		case 'neon':
		case 'greenneon':
		...
		case 'thunder':
			...
			setReply(mess.wait)
			ehz.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/textprome/${command}?apikey=${api.Lol}&text=${args}` }, caption: `© Maleficent Botz\n\n Type: ${command}\n\nText: ${args}`})
			break
case 'cyberspace':
            case 'mountain':
            case 'programming':
            ...
            setReply(mess.wait)
                ...
                ehz.sendMessage(m.chat, { image: { url: result}, caption: mess.succes }, { quoted: ftoko })
            }
            break
...
if (!isGroup) return onlyGroup()
                ...
                ehz.sendMessage(m.chat, { image: { url: random.female }, caption: `Couple Female` }, { quoted: ftoko })
            }
	    break	    
        case 'pantun': {
if (!isGroup) return onlyGroup()
...
if (!isGroup) return onlyGroup()
let rangel = await fetch(`https://api.betabotz.eu.org/api/random/${command}?apikey=2fbgCgOB`)
...
var hasil = pickRandom(notnot)
ehz.sendMessage(m.chat, { caption: mess.success, image: { url: hasil.url } }, { quoted: ftoko })
break
case 'hijab':
setReply(mess.wait)
...
let encmedia = await ehz.sendImageAsSticker(from, media, ftoko, { packname: global.packName, author: global.authorName })
await fs.unlinkSync(encmedia)
} else if (/video/.test(mime)) {
if ((quoted.msg || quoted).seconds > 11) return setReply('Maksimal 10 detik!')
let media = await quoted.download()
let encmedia = await ehz.sendVideoAsSticker(from, media, ftoko, { packname: global.packName, author: global.author })
await fs.unlinkSync(encmedia)
} else {
...
let encmedia = await ehz.sendImageAsSticker(m.chat, buffer, ftoko, { packname: global.packName, 
author: '  Maleficent', 
categories: ['', ''],
id: '12345',
quality: 100,
...
ehz.sendMessage(owner + "@s.whatsapp.net",{image: { url: `https://telegra.ph/file/048acba09543c168246e5.jpg`}, caption: text12, mentionedJid:[sender], quoted: ftoko })
}
break
case 'accpesanan': {
if (!isOwner && !isAdmins) return setReply()
...
await ehz.sendMessage(from, { document: fs.readFileSync("./database/contacts.vcf"), fileName: "contacts.vcf", caption: "Sukses Tinggal Save Ya Kakak", mimetype: "text/vcard", }, { quoted: ftoko })
contacts.splice(0, contacts.length)
fs.writeFileSync("./database/contacts.json", JSON.stringify(contacts))
}
}
...
ehz.sendMessage(m.chat, { react: { text: "⏳", key: m.key, }})
  ...
  await ehz.sendMessage(m.chat, { video: { url: json.data.play }, caption: fbk }, { quoted: ftoko })
